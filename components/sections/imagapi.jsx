...
import FaImages from '@meronex/icons/fa/FaImages'; // or FaLayerGroup, FaBox, etc.

// List of asset types from your API
const ASSET_TYPES = [
...
// Your own ImageKit images (add as many as you want)
const MY_IMAGEKIT_IMAGES = [
  { name: 'Location Pin', filename: 'location.png' },
  { name: 'Store Icon', filename: 'store-icon.png' },
...
export const ImagApiPanel = observer(({ store }) => {
  ...
  const fetchAssets = async () => {
    if (isMyImages) {
      ...
      const formatted = MY_IMAGEKIT_IMAGES.map(img => ({
        src: `https://ik.imagekit.io/ericmwangi/${img.filename}?tr=w-200,h-200`, // small preview
  ...
  const handleTypeChange = (e) => {
    ...
    // Reset search to default for this type
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: 10 }}>
      ...
      {/* Search input (hidden for My ImageKit section) */}
      {!isMyImages && (
        <InputGroup
          leftIcon="search"
      ...
      <ImagesGrid
        ...
        isLoading={loading}
        onSelect={img => {
          // Add as regular image (you can customize size per type)
